...
public class EmployeeController {
	...
	public String searchEmployee(@RequestParam(name = "empId") int empIdVal, ModelMap modelMap) {
		EmployeeBean bean = employeeService.getEmployeeDetailById(empIdVal);
		if (bean != null) {
	...
	@PostMapping("/addEmp")
	public String createEmployeeInfo(EmployeeBean bean, ModelMap modelMap) {

		boolean isAdded = employeeService.createEmployeeInfo(bean);
	...
	public String updateEmployee(EmployeeBean bean, ModelMap modelMap) {
		...
		boolean updated = employeeService.updateEmployeeInfo(bean);
		if (updated) {
	...
	public String deleteEmployee(int empId ,ModelMap modelMap) {
		...
		boolean updated=employeeService.deleteEmployeeInfo(empId);
		if(updated) {
	...
	public String allEmployee(EmployeeBean bean,ModelMap modelMap) {
		...
		List<EmployeeBean> bean1=employeeService.getAllEmployeesDetails();
